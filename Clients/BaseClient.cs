...
public class BaseClient : IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        if (requiresAuth)
        {
            ...
            {
                _logger.LogInformation("Token missing or expired. Authenticating...");
        ...
        while (attempt < maxRetries)
        {
            ...
            try
            {
                ...
                // Log with retry metrics
                _loggingHelper.LogRequestAndResponse(_logger, _client, request, response, stopwatch.ElapsedMilliseconds, GetToken(), attempt, maxRetries);
            ...
            {
                lastException = ex;
                _logger.LogWarning(ex, "Attempt {Attempt} failed. Retrying in {Delay} ms...", attempt, 500 * attempt);
    ...
    {
        var (raw, elapsedMs) = await ExecuteAsync(request, requiresAuth, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var authResponse = JsonHelper.DeserializeSafe<AuthResponse>(response.Content, "Failed to authenticate.");
